...
const PositionTable = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error(err.response?.data?.message || "Đã xảy ra lỗi khi xóa vị trí.");
    } finally {
          ...
          <div className="table-responsive">
            <table className="table table-hover align-middle mb-0">
              <thead className="table-light">
                <tr>
                  <th className="ps-4 py-3" style={{ width: "200px" }}>Mã ID</th>
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan="3" className="text-center py-3">Đang tải dữ liệu...</td>
                  </tr>
                ) : error ? (
                  <tr>
                    <td colSpan="3" className="text-center py-3 text-danger">{error}</td>
                  </tr>
                ) : positions.length === 0 ? (
                  <tr>
                    <td colSpan="3" className="text-center py-3 text-muted">Không có dữ liệu</td>
                  </tr>
                ) : (
                  positions.map((position) => (
                    <tr key={position.id}>
                      <td className="ps-4">
                        <span className="badge text-dark" style={{ fontSize: 16 }}>{position.id}</span>
                      </td>
                      <td>
                      ...
                      <td className="text-end pe-4">
                        <div className="d-flex align-items-center justify-content-end gap-2">
                          <Link
                            to={`/vi-tri/danh-sach/${position.id}`}
