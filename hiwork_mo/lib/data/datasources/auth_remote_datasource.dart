...
class AuthRemoteDataSourceImpl implements AuthRemoteDataSource {
  ...
  Future<UserModel> signIn({
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      final empJson = response.data["user"]["employee"] as Map<String, dynamic>;
      await EmployeeDetailStorage().saveEmployeeJson(empJson);
  ...
  Future fetchEmployeeIdByUserId(userId) async {
    final response = await _dio.get("${ApiUrl.employees}/by_user/$userId");
    if (response.statusCode == 200 || response.statusCode == 201) {
